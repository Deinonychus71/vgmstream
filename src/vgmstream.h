...
__declspec(dllexport) VGMSTREAM* init_vgmstream(const char* const filename);

/* init with custom IO via streamfile */
__declspec(dllexport) VGMSTREAM* init_vgmstream_from_STREAMFILE(STREAMFILE* sf);

/* reset a VGMSTREAM to start of stream */
__declspec(dllexport) void reset_vgmstream(VGMSTREAM* vgmstream);

/* close an open vgmstream */
__declspec(dllexport) void close_vgmstream(VGMSTREAM* vgmstream);

/* calculate the number of samples to be played based on looping parameters */
__declspec(dllexport) int32_t get_vgmstream_play_samples(double looptimes, double fadeseconds, double fadedelayseconds, VGMSTREAM* vgmstream);

/* Decode data into sample buffer. Returns < sample_count on stream end */
__declspec(dllexport) int render_vgmstream(sample_t* buffer, int32_t sample_count, VGMSTREAM* vgmstream);

/* Seek to sample position (next render starts from that point). Use only after config is set (vgmstream_apply_config) */
__declspec(dllexport) void seek_vgmstream(VGMSTREAM* vgmstream, int32_t seek_sample);

/* Write a description of the stream into array pointed by desc, which must be length bytes long.
 * Will always be null-terminated if length > 0 */
__declspec(dllexport) void describe_vgmstream(VGMSTREAM* vgmstream, char* desc, int length);
__declspec(dllexport) void describe_vgmstream_info(VGMSTREAM* vgmstream, vgmstream_info* desc);

/* Return the average bitrate in bps of all unique files contained within this stream. */
__declspec(dllexport) int get_vgmstream_average_bitrate(VGMSTREAM* vgmstream);
...
__declspec(dllexport) const char** vgmstream_get_formats(size_t* size);

/* same, but for common-but-disabled formats in the above list. */
__declspec(dllexport) const char** vgmstream_get_common_formats(size_t* size);
...
__declspec(dllexport) void vgmstream_force_loop(VGMSTREAM* vgmstream, int loop_flag, int loop_start_sample, int loop_end_sample);

/* Set number of max loops to do, then play up to stream end (for songs with proper endings) */
__declspec(dllexport) void vgmstream_set_loop_target(VGMSTREAM* vgmstream, int loop_target);

/* Return 1 if vgmstream detects from the filename that said file can be used even if doesn't physically exist */
__declspec(dllexport) int vgmstream_is_virtual_filename(const char* filename);
...
__declspec(dllexport) VGMSTREAM* allocate_vgmstream(int channel_count, int looped);

/* Prepare the VGMSTREAM's initial state once parsed and ready, but before playing. */
__declspec(dllexport) void setup_vgmstream(VGMSTREAM* vgmstream);

/* Open the stream for reading at offset (taking into account layouts, channels and so on).
 * Returns 0 on failure */
__declspec(dllexport) int vgmstream_open_stream(VGMSTREAM* vgmstream, STREAMFILE* sf, off_t start_offset);
__declspec(dllexport) int vgmstream_open_stream_bf(VGMSTREAM* vgmstream, STREAMFILE* sf, off_t start_offset, int force_multibuffer);

/* Get description info */
__declspec(dllexport) void get_vgmstream_coding_description(VGMSTREAM* vgmstream, char* out, size_t out_size);
__declspec(dllexport) void get_vgmstream_layout_description(VGMSTREAM* vgmstream, char* out, size_t out_size);
__declspec(dllexport) void get_vgmstream_meta_description(VGMSTREAM* vgmstream, char* out, size_t out_size);

__declspec(dllexport) void setup_state_vgmstream(VGMSTREAM* vgmstream);

/* Changes for Sm5shMusic */
__declspec(dllexport) int32_t get_vgmstream_samplerate(VGMSTREAM* vgmstream);

__declspec(dllexport) int get_vgmstream_channel_number(VGMSTREAM* vgmstream);

__declspec(dllexport) int32_t get_vgmstream_loop_startsample(VGMSTREAM* vgmstream);

__declspec(dllexport) int32_t get_vgmstream_loop_endsample(VGMSTREAM* vgmstream);

__declspec(dllexport) int32_t get_vgmstream_totalsamples(VGMSTREAM* vgmstream);
#endif